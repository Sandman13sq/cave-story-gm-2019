...
{
	if weaponHasAmmo(Weapon.superLauncher)
	{
		if (projectileCount + 3) <= projectileMax
		{
			var _dir = direction;
			...
			repeat(3)
			{
					...
					self)
				{
					...
					otherSpeed = random(2);
					set_projectile_sprite(spr_proj_sMissileSmall, 3);
				}
			}
			...
			with SOURCE
			{
				...
				addWeaponAmmo(Weapon.superLauncher, -1);
			}
		}
	}
